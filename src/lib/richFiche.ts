...
- Chimie, structures (le dessin topologique est fait par l'app à partir d'un SMILES) : \`<span class="mol" data-smiles="CC(=O)O" data-name="acide éthanoïque"></span>\` (un composé par balise, hydrogènes implicites, \`data-name\` facultatif). Schéma réactionnel : \`<div class="scheme">\` contenant des \`mol\`, des \`<span class="plus">+</span>\` et des flèches \`<span class="arrow" data-label="H₂SO₄, Δ"></span>\` (\`arrow eq\` pour un équilibre). Vérifie chaque SMILES (valences, cycles fermés). Les flèches courbes ne sont pas dessinées : décris le déplacement des doublets dans le texte.
