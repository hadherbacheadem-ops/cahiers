...
const TYPE_LINES: Partial<Record<ExerciseType, string>> = {
  ...
  mecanisme: `- "mecanisme" (chimie organique, mécanismes réactionnels : UNIQUEMENT si la fiche en contient) : un mécanisme découpé en 2 à 8 étapes élémentaires ("steps"). "statement" = la réaction ou le mécanisme à analyser. Chaque étape : "text" (ce qui se passe, en mots, SANS nommer le type de réaction), "reactants" et "products" (listes de SMILES valides des espèces avant et après l'étape : l'application les dessine en formules topologiques ; hydrogènes implicites, charges entre crochets : "[OH-]", "[Br-]", "C[C+](C)C" ; vérifie les valences), "conditions" (réactifs et conditions au-dessus de la flèche, optionnel), "answer" = le type de réaction de l'étape, exactement l'un de : "SN1", "SN2", "E1", "E2", "AdN" (addition nucléophile), "AdE" (addition électrophile), "AdN-E" (addition-élimination), "SEAr", "acide-base", "redox", "radicalaire", "pericyclique", "transposition", "complexation" ; "alsoAccept" (optionnel) = autres types acceptés pour l'étape ; "explanation" = pourquoi (nucléophile, nucléofuge, stéréochimie, intermédiaire). L'élève donne le type de CHAQUE étape : varie-les quand le mécanisme s'y prête (protonation acide-base, puis SN1, puis déprotonation acide-base). Un exercice par mécanisme de la fiche.`,
...
${input.prepaSection ? 'Le JSON contient AUSSI la clé "preparation" (voir plus haut), à côté de "points" et "exercises".\n' : ''}UNIQUEMENT un bloc \`\`\`json, sans texte autour, conforme à ce schéma (exemple de formule bien écrite : "answer": "$E_c = \\\\frac{1}{2} m v^2$") :
{
  ...
  "exercises": [
    ...
    { "pointId": "p5", "type": "mecanisme", "title": "…", "statement": "…", "steps": [ { "text": "…", "reactants": ["CC(C)(C)Br"], "products": ["C[C+](C)C", "[Br-]"], "conditions": "éthanol", "answer": "SN1", "explanation": "…" }, { "text": "…", "reactants": ["C[C+](C)C", "CCO"], "products": ["CC(C)(C)[O+](H)CC"], "answer": "SN1" } ], "difficulty": 3, "tags": ["…"] },
