...
const base = { difficulty, tags }
const str = z.string().trim().min(1)

const flashcard = z.object({ type: z.literal('flashcard'), question: str, answer: str, hint: z.string().optional(), ...base })
...
const exerciseSchema = z.discriminatedUnion('type', [flashcard, cloze, mcq, truefalse, match, order])

const outputSchema = z.object({ exercises: z.array(z.unknown()) })
...
export function parseClaudeResponse(text: string): ParseResult {
  ...
  const list = Array.isArray(parsed) ? parsed : outputSchema.safeParse(parsed).data?.exercises
  if (!list) throw new Error('Le JSON ne contient pas de tableau "exercises".')
  ...
  list.forEach((item, index) => {
    ...
    const { difficulty, tags, ...data } = res.data
    exercises.push({ data, difficulty, tags })
  })
