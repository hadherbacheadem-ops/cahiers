...
/** For each `{{…}}` blank in order, whether it sits inside `$…$` (same scan as cloze.ts `blankInsideMath`). */
function blankMathFlags(text: string): boolean[] {
  const flags: boolean[] = []
  let inMath = false
  let i = 0
  while (i < text.length) {
...
/** Deterministic GUID (genanki's `guid_for` scheme): base91 of the first 64 bits of a hash. */
...
function template(name: string, ord: number, qfmt: string, afmt: string) {
  return { name, ord, qfmt, afmt, bqfmt: '', bafmt: '', did: null as number | null }
}
