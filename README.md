...
4. **S'entraîner** — *Réviser* (uniquement ce qui est dû, répétition espacée), *S'entraîner* (tout), *Chrono* (compte à rebours, N questions).
   Pas de nombre d'exercices imposé : tu coches les types autorisés et le prompt demande *autant d'exercices qu'il y a de points de cours, aussi petits soient-ils*. Claude liste d'abord les points de cours (avec une citation exacte de la fiche), puis les exercices rattachés ; l'app valide le JSON, passe chaque exercice au linter, repère les doublons et te les présente dans une file « à valider » (J / K / E). Formules en LaTeX rendues partout.
   Le planificateur est **FSRS** (ts-fsrs) : rétention visée réglable, limites journalières, jours légers, annulation, reporter / avancer. Voir `DESCRIPTIF.md` et `DECISIONS.md`.
