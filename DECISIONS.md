...
## Chimie : formules topologiques et exercice « mécanisme »
- Les structures s'écrivent en SMILES (Claude sait les écrire) et l'app les dessine avec `smiles-drawer` (MIT, auto-hébergé, 57 Ko gzip, chunk chargé à la première molécule : hors démarrage). Pas de service externe. Longueur de liaison fixe (échelle 1) pour qu'une petite molécule reste petite à côté d'une grande ; dégradés de liaison remplacés par un trait plein ; carbone et liaisons en `currentColor` (suit le thème), hétéroatomes colorés. Un SMILES illisible reste affiché en texte rouge : une faute se voit.
- Dans une fiche riche : `<span class="mol" data-smiles="…" data-name="…">`, `<div class="scheme">` avec `plus` et `arrow` (`arrow eq` pour l'équilibre). Le texte tiré de la fiche (`htmlToText`) garde le SMILES, pour que les exercices puissent porter sur la structure. Non fait : les flèches courbes de mécanisme, et les molécules dans les fiches et exercices en markdown.
- Type d'exercice `mecanisme` : un mécanisme découpé en étapes ; l'élève donne le type de réaction de chaque étape (SN1, SN2, E1, E2, AdN, AdE, AdN-E, SEAr, acide-base, redox, radicalaire, péricyclique, transposition, complexation). Vocabulaire fermé (`lib/reactionTypes.ts`) : les graphies de Claude (« AN », « ADN », « acido-basique »…) sont ramenées à l'identifiant, un type inconnu rejette l'exercice à l'import. Juste seulement si toutes les étapes le sont ; `alsoAccept` pour les étapes ambiguës. Proposé dans les types à générer, mais absent des types cochés par défaut (utile seulement en chimie organique).
- Test de budget de démarrage : première exécution après un build parfois à 224 Ko au lieu de 175 (les chunks chargés à l'inactivité passent avant l'évènement `load`) ; l'ordre dépend du timing, pas du code.
